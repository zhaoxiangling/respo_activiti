...
layui.define(["element","jquery"],function(exports){
	...
	//通过name获取lay-id
	Tab.prototype.getLayId = function(name){
		$(".layui-tab-title.top_tab li").each(function(){
			if($(this).find("cite").text() == name){
	...
	//通过name判断tab是否存在
	Tab.prototype.hasTab = function(name){
		var tabIndex = -1;
		$(".layui-tab-title.top_tab li").each(function(){
			if($(this).find("cite").text() == name){
				tabIndex = 1;
		...
		if(_this.attr("target") == "_blank"){
			...
			if(_this.find("i.seraph,i.layui-icon").attr("data-icon") != undefined){
				if(_this.find("i.seraph").attr("data-icon") != undefined){
					name += '<i class="seraph '+_this.find("i.seraph").attr("data-icon")+'"></i>';
				}else{
					name += '<i class="layui-icon">'+_this.find("i.layui-icon").attr("data-icon")+'</i>';
				}
			}
			//已打开的窗口中不存在
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				tabIdIndex++;
				name += '<cite>'+_this.find("cite").text()+'</cite>';
				name += '<i class="layui-icon layui-unselect layui-tab-close" data-id="'+tabIdIndex+'">&#x1006;</i>';
			...
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					...
					for(var i=0;i<menu.length;i++){
						if($("#top_tabs li.layui-this cite").text() == menu[i].name){
